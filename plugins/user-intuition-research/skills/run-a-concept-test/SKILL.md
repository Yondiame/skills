...
1. **Create the study with `is_concept_test: true`** — follow the create-study-from-brief flow, but draft the moderator guide from the concept-test template (the study-type row from `list_study_types`, or the `concept_test_planning_context` / `concept_test_chat_planning_context` prompt as fallback).
